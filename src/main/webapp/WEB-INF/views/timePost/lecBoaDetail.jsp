...
$(function() {
   ...
   // $.ajax({
   //    url:"/timePost/lecBoaDetailPostAjax", // URL 경로에 파라미터 추가,
   //    contentType : "application/json;charset=utf-8",
   //    data : JSON.stringify(timeLecBoNoData),
   //    type : "post",
   //    dataType : "json",
   //    beforeSend: function(xhr){
   //                    xhr.setRequestHeader("${_csrf.headerName}", "${_csrf.token}");
   //                },
   //    success:function(result){
   //       console.log("lecBoaDetailGetAjax result ",result)
   //    }
   // })
...
function timeBoastComm(){
   ...
   $.ajax({
      ...
      success: function(result) {
      let commCount= result.timeBoastCommVOList.length;
      let timeBoastComm ="";
      $.each(result.timeBoastCommVOList,function(idx,timeBoastCommVO){
         // console.log("timeBoastCommVO",timeBoastCommVO.timeBoaCommNo)

         //내가 작성한 경우
         if (dataRole.includes("STUDENT") && userLoggedIn == timeBoastCommVO.stNo){       
            timeBoastComm+=`
                     <tr class="commentBor" style="background-color: #ebf1e9">
                        <td class="tdPaddingNic" style="width: 70%;">익명(작성자)</td>
         ...
         //다른사람이 작성한경우
            ...
            timeBoastComm+=`
                     <tr class="commentBor" style="background-color: #ebf1e9">
                        <td class="tdPaddingNic" style="width: 70%;">익명\${commIdx}`;
         ...
         timeBoastComm+=`<td class="tdPaddingBut">
                           <button type="button" class="btn btn-block btn-outline-danger btn-xs btncliDecl" id="btnBlind">블라인드</button>
                        </td>
                        `;
          }
...
<body>
   ...
   <div>
               ...
               <div class="btnbtn row" style="margin: auto;">
                  <p id="p1">
                     ...
                     <sec:authorize access="hasRole('ROLE_ADMIN')">
                        <button type="button" class="btn btn-block btn-outline-danger btncli" id="btnBlind">블라인드</button>
                     </sec:authorize>
                     <sec:authorize access="hasRole('ROLE_STUDENT') and !principal.username.equals('${timeLecutreBoastVO.stNo}')">
                        <div id="boastDeclarationDisp">
                           <button type="button"class="btn btn-block btn-outline-danger btncli" id="btnDeclaration" style="height: 40px;">신고</button>
                        </div>
                     </sec:authorize>
                     <sec:authorize access="hasRole('ROLE_STUDENT') and principal.username.equals('${timeLecutreBoastVO.stNo}')">
                        <button type="button"class="btn btn-block btn-outline-danger btncli" id="btnDelete">삭제</button>
                     </sec:authorize>
                  </p>
                  <p id="p2" style="display: none;">
                     <button type="button" class="btn btn-block btn-outline-primary btncli" id="btnSave">확인</button>
                     <button type="button" class="btn btn-block btn-outline-secondary btncli" id="cancel">취소</button>
                  </p>
               </div>
            </div>
         </div>
      ...
      <div class="mainbdFoot card card-primary card-outline">
         <div>
            <h5 class="queH4 far fa-comments">댓글 <span id="commCountDisp">${timeLecutreBoastVO.timeLecLike}</span></h5>
         </div>
